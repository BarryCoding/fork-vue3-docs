...
Bundlers like Rollup (which Vite is based upon) can automatically create split chunks by detecting the **ESM dynamic import syntax**:
...
Lazy loading is best used on features that are not immediately needed after initial page load. In Vue applications, this can be used in combination with Vue's [Async Component](/study/components/async) feature to create split chunks for component trees:
