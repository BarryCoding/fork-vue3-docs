...
`watch`'s first argument can be different types of reactive "sources": it can be 
1. a ref (including computed refs) or a reactive object
2. a [getter function](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/get#description)
3. an array of multiple sources:
...
Watchers declared synchronously inside `<script setup lang='ts'>` are bound to the owner component instance, and will be automatically stopped when the owner component is unmounted. In most cases, you don't need to worry about stopping the watcher yourself.
