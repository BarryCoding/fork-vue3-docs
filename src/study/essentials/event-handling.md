...
:::tip
Keep in mind that event names provide automatic case transformation. It’s recommend to use kebab-cased event listeners in templates.
:::
...
- `foo`, `foo.bar` and `foo['bar']` are treated as method handlers,
- `foo()` and `count++` are treated as inline handlers.
...
## Keyboard Modifiers {#key-modifiers}
